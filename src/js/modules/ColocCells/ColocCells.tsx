                ...
                <div className={`grid quarter-gap cols-${curParams.cellsperrow[0]}`}>
                    {result.imgs.map((r,i)=>
                        <ColocCellResult foci0={result.foci[0][i]}
                                         ...
                                         foci1={result.foci[1][i]} onToggleCellInclusion={()=>onCellInclusionToggle(i)} excluded={selected.indexOf(i) == -1}
                                         cnt={result.cnts[i]} imgIdx={selMod} key={i} res={r}/>)}
                </div>
                <Alert severity="info" className={'margin-100-top'}>
                    Be aware that cells that have close to no signal will appear in full brightness for that channel, since all cells are normalized in intensity.
                    This will make it look like colocalization appears but is in actuality nothing.
                    The final colocalization detection however is not based on pixels but on the detected foci and the overlap of their outlines. This problem will therefore not occur.
                </Alert>
            </>
        }
	</div>);
