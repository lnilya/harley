import {Button, Dialog, FormControl, NativeSelect} from "@mui/material";
...
const PipelineListEntry: React.FC<IPipelineListEntryProps> = ({groupName, group, onChoose}) => {
    ...
    return (
        <div className={'rel'}>
            <PipelineDebugDropdown pipe={pl}/>
            <div className={'pipeline-list-entry margin-200-bottom full-w ' + cl(isGroup, 'is-grouped')}>
                <div className="fl-row bg-bglight">
                    ...
                    <div className={'pipeline-list-entry__text pad-100 fl-grow'}>
                        {isGroup &&
                        <div
                            className="fl-row-start margin-100-neg-exceptbottom margin-100-bottom pipeline-list-entry__group-selection">
                            {Object.keys(group).map((grname) => {
                                return <div key={grname} onClick={() => setGroupSelection(grname)}
                                            className={`homolog pad-25-ver pad-50-hor ${cl(grname == groupSelection, 'active')}`}>{grname}</div>
                            })}
                        </div>
                        }
                ...
                <div className="margin-50-top text-right fl-row-end">
                    ...
                    <>
                        <Dialog open={showingHelp} onClose={e => setShowingHelp(false)} maxWidth={'md'}
                                fullWidth={true}>
                        ...
                        <Button variant={"outlined"} color={'secondary'} onClick={e => setShowingHelp(true)}>Show
                            Tutorial</Button>
                    ...
                    <Button variant={"contained"} color={'primary'}
                            onClick={() => onChoose(pl)}>Start {pl.name}</Button>
                </div>
            </div>
        </div>);
}
